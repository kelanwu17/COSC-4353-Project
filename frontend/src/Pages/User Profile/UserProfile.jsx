...
const UserProfile = () => {
  ...
  useEffect(() => {
    ...
    try {
        ...
        if (response.data) {
          ...
          const { email, address, address2, city, zipcode, fullName, state, selectedSkills } = response.data;
          const skillsArray = selectedSkills ? selectedSkills.split(',').map(skill => skill.trim()) : [];
  ...
  const updateProfile = async (event) => {
    ...
    try {
        const response = await axios.put(`http://localhost:3001/updateprofile/${username}`, profileData);
        if (response) {
...
const deleteProfile = async (event) => {
  ...
  try {
      const response = await axios.delete(`http://localhost:3001/deleteprofile/${username}`);
      if (response) {
        ...
        <form >
          <div className="edit-profile-section">
            ...
            <div className="profile-state">
              ...
              <select onChange={(e) => setState(e.target.value)} id="state" name="state" required value={state}>
...
/>
          ...
          <div className="apply-button">
            <Button onClick={updateProfile}  variant="contained" color="primary">Apply Changes</Button>

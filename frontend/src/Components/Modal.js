...
export default function Modal({ open, children, onClose, title, description, urgency, skills, location, date }) {
    ...
    return ReactDOM.createPortal( <
        ...
        div style = {
            { overflowWrap: 'break-word', wordBreak: 'break-word' } } > { description } < /div> <
        div > Urgency: { urgency } < /div> <
        div style = {
            { overflowWrap: 'break-word', wordBreak: 'break-word' } } > Skills Required: {
        ...
        <
        div > Date: { date } < /div> <
        button className = "rounded-sm border border-black bg-blue-400 text-white absolute bottom-2 right-18 "
        onClick = {
            (e) => { e.stopPropagation();
                onClose(); } } > Close < /button> <
        button className = "rounded-sm border border-black bg-blue-400 text-white absolute bottom-2 right-2"

...
export default function AdminModal({ open, onClose, title, description, urgency, skills, location, date }) {
    ...
    return ReactDOM.createPortal( <
        ...
        value = { newLoc }
        onChange = {
            (temp) => changeLoc(temp.target.value) }
        /> <
        /div> <
        div className = "text-xs" >
        Date:
        ...
        value = { newDate }
        onChange = {
            (temp) => changeDate(temp.target.value) }
        /> <
        /div> <
        div >
        <
        label className = "block text-sm" > Title: < /label> <
        textarea className = "border border-black border-2"
        value = { newTitle }
        onChange = {
            (temp) => changeTitle(temp.target.value) }
        /> <
        /div> <
        div >
        <
        label className = "block text-sm" > Description: < /label> <
        textarea className = 'w-full border border-black border-2'
        style = {
            { overflowWrap: 'break-word', wordBreak: 'break-word' } }
        value = { newDesc }
        onChange = {
            (temp) => changeDesc(temp.target.value) }
        /> <
        /div> <
        div >
        <
        label className = "block text-sm" > Urgency: < /label> <
        select className = "border border-black border-2"
        value = { newUrg }
        onChange = {
            (temp) => changeUrg(temp.target.value) } > {
        ...
        <
        button className = "rounded-md bg-blue-400 text-white border border-black right-0"
        onClick = {
            (temp) => { temp.stopPropagation();
                onClose(); } } >
        Save <
        /button> <
        /div> <
